...
const actualUserSlice = createSlice({
  ...
  reducers: {
    ...
    deleteFromCart: (state, action) => {
      state.actualUser.cart = state.actualUser.cart.filter((_, index) => action.payload !== index)
      localStorage.setItem("actualUser", JSON.stringify(state.actualUser));
    },
...
export const { addActualUser, addToCart, deleteFromCart, changeTotalSum } = actualUserSlice.actions;
